...
### 2026-07-01 — 2.9B 是 8GB fp16 断点
2.9B(权重 ~5.8GB)在 5070(8GB)上跑 alignment 段错误(exit 139,OOM:权重+chunk workspace 超 8GB)。convert 正常(`--no-fuse-norm`)。**0.1B/0.4B/1.5B fp16 能跑,2.9B 起 fp16 装不下 → 必须量化**(int4 下 ~3GB 才进得去 8GB)。这正好量化了 E1/E2 的必要性:**2.9B/7.2B/13.3B 在消费级 8GB 卡上要能跑,量化加速路径(E1/E2,目前慢 13×)是硬刚需**,不是可选项。
