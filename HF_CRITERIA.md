...
| 门禁 | 目标 | 度量 | 现状 |
|---|---|---|---|
| E1/E2 量化 decode | 显存↓、速度≥fp16(req-5) | 单 token decode TPOT | ❌ **量化比 fp16 慢 ~13×**:4bit nf4 TPOT **45ms(22 tok/s)** vs fp16 3.54ms(283 tok/s)。根因:fast-token 拒绝量化→FLA 回退(无快 decode)+ bnb 4bit 每 op 反量化。要达标需「量化版 native_graph runner」。附:1D 输入在量化路径会崩(2D 正常,真实 generate 传 2D 所以不影响)|
