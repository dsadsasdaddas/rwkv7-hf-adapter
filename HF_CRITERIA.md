...
> **度量标准(以后所有性能数字都按这个)**:不能只报"tok/s"。服务性能必须拆成
> **TTFT**(prefill 首 token 延迟)+ **TPOT**(decode 每 token 延迟),报 **p50/p90/p99**;
> 在**定义好的输入/输出长度分布**下;用下面两种框架之一。

**长度 profile(固定基准负载,不再随手选)**:
- `ISL`(input seq len)档:128 / 512 / 2048;`OSL`(output seq len)档:64 / 128 / 512。
- smoke 用 (ISL=512, OSL=128);服务级跑全网格。

**两种实验框架**:
- **固定并发**:并发 = {1,2,4,8,16},报 TTFT/TPOT p50/p99 + 总吞吐(tok/s)。看随并发退化。
- **固定 SLO**(硬指标):给定 TTFT≤X ms、TPOT≤Y ms,找**最大可承载并发**。这才是"性能"。

| 门禁 | 目标 | 度量 | 现状 |
|---|---|---|---|
| B1 TTFT(prefill 首 token 延迟) | p99 ≤ 1.1× Albatross @ 同 ISL | TTFT bench(prefill 计时)**待补** | ⚠️ 仅 prefill tok/s 粗数(HF >> 官方 torch-ref),无 TTFT p99 |
| B2 TPOT(decode 每 token,bsz=1) | p50 ≤ 1.1× Albatross | decode ms/tok p50 | ⚠️ 单流数有(V100 native_graph 255→~3.9ms;5070 395→~2.5ms;vs RWKV-LM 真 TPOT ~10ms),**但只 bsz=1,无 p99** |
| B3 并发扫描(固定并发 TTFT/TPOT) | 1/2/4/8/16 各档不退化 | 并发扫描 bench **待补** | ❌ 只有固定 batch 的聚合 tok/s,不是并发-延迟曲线 |
| B4 最大并发 @ SLO | TPOT≤Y 下并发 ≥ N(对标 Albatross) | SLO 扫描 **待补** | ❌ 完全没做 |
| B5 generate 默认即快 | `model.generate` 走 fast-token | `bench/bench_generate_fast_path.py` | ✅ `RWKV7_FAST_FORWARD=1` 默认路由 |

**关键缺口**:main 的 bench(`bench_speed`/`bench_native_decode`/`bench_batch*`)报的是**固定长度固定 batch 的聚合 tok/s + ms/tok**,是**单发/小批量 smoke**,不是服务级。**没有 TTFT/TPOT 拆分、没有 p99、没有并发-延迟曲线、没有 SLO 最大并发**。要真和 Albatross(它的 benchmark 是 B×T 网格 + 百分位延迟)比服务性能,必须**自建一套 TTFT/TPOT×并发 度量**(参照 Albatross `faster2/benchmark.py` 的 B×T 网格方法)。这是 req-1 性能侧**比"单流快几倍"重要得多**的真缺口。
