...
| 门禁 | 目标 | 度量 | 现状 |
|---|---|---|---|
| E1/E2 量化 decode | 显存↓、速度≥fp16 | 单 token decode TPOT | ❌ **量化(4bit)单 token decode 直接崩**:`fla/layers/rwkv7.py:233` 收到 2D hidden(应 3D)——量化模型 use_cache 单步路径 bug。fp16 baseline TPOT 3.54ms(283 tok/s)。量化 TPOT 暂时量不了,需先修此 bug |
