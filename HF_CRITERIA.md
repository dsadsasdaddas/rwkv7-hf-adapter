...
| 门禁 | 目标 | 度量 | 现状 |
|---|---|---|---|
| B1 TTFT(单次 prefill 延迟) | p99 ≤ 1.1× RWKV-LM/Albatross @ 同 ISL | TTFT bench(prefill 计时)**待补** | ⚠️ 仅 prefill tok/s 粗数(HF >> 官方 torch-ref),无 TTFT p99 |
| B2 TPOT(单次 decode/token) | p50 ≤ 1.1× RWKV-LM/Albatross | decode ms/tok p50 | ⚠️ bsz=1 数有(V100 native_graph ~3.9ms;5070 ~2.5ms;vs RWKV-LM 真 TPOT ~10ms),无 p99 |
| B3 batch generate 吞吐 | `model.generate(batch_size=B)` B=1/4/8 近线性、≥ 官方 | batch generate bench | ⚠️ 有固定 batch 的聚合 tok/s(0.1B B=16→364),非 generate 路径专门测 |
