...
if _HAS_TRITON:
    ...
    @triton.jit
    def _compact_wy_recompute_phase_probe_kernel(
        r_ptr,
        w_ptr,
        k_ptr,
        v_ptr,
        ...
        """Micro/probe phases for the compact recompute-starts kernel.

        ``MODE=0`` runs the duplicated compact-prefix recompute loop only and
        stores a small row checksum.  ``MODE=1`` skips prefix recompute and runs
        only the per-token apply/output loop from the initial state.  The probe
        is intentionally not a production path; it keeps the work shape close
        to ``_compact_wy_recompute_apply_output_kernel`` so benchmark rows can
        decide the next kernel boundary from measured prefix-vs-token costs.
        """
        ...
        if MODE == 0:
            ...
            probe_base = ((batch_id * CHUNKS + chunk_id) * H + head_id) * N
            checksum = tl.sum(cur, axis=1)
            tl.store(prefix_probe_ptr + probe_base + offs_i, checksum, mask=mask_i)
    ...
    @triton.jit
    def _compact_wy_grouped_prefix_shared_apply_output_kernel(
        r_ptr,
        w_ptr,
        k_ptr,
        v_ptr,
        ...
        """Grouped prefix-shared schedule probe.

        One program owns `(batch, group, head, row_block)`.  It computes the
        group's start state once from compact-WY prefix factors, then applies
        `GROUP_SIZE` consecutive chunks serially.  This is the bounded
        parallelism-recovery experiment between the two extremes:

        * `GROUP_SIZE=1`: chunk-parallel but duplicated prefix per chunk;
        * `GROUP_SIZE=CHUNKS`: no duplicated prefix but no chunk parallelism.
        """
        ...
        tmp = pid // ROW_BLOCKS
        head_id = tmp % H
        group_id = (tmp // H) % GROUPS
        batch_id = tmp // (H * GROUPS)
        ...
        while prefix_chunk < CHUNKS:
            ...
            next_cur += tl.dot(trans_coeff, tl.trans(trans_right), input_precision="ieee")
            next_cur += tl.dot(add_left, tl.trans(add_right), input_precision="ieee")
            cur = tl.where(prefix_chunk < group_start, next_cur, cur)
            prefix_chunk += 1

        group_local = 0
        while group_local < GROUP_SIZE:
            chunk = group_start + group_local
            local_i = 0
            while local_i < C:
                t = chunk * C + local_i
                vec_base = ((batch_id * T + t) * H + head_id) * N
        ...
        final_mask = row_mask & (group_id == GROUPS - 1)
        tl.store(final_state_ptr + state_base + offs_i[:, None] * N + offs_j[None, :], cur, mask=final_mask)
...
def dplr_compact_wy_recompute_phase_probe_triton(
    r: Any,
    w: Any,
    k: Any,
    v: Any,
    ...
    *,
    ...
    """Run a timing-only micro phase of compact recompute-starts.

    ``phase="prefix"`` runs only the duplicated compact-prefix recomputation
    work from :func:`dplr_compact_wy_recompute_apply_output_triton` and stores
    a compact row checksum.  ``phase="token_apply"`` skips prefix recompute and
    runs only the token apply/output loop from the initial state.  These probes
    are deliberately not correctness routes; they estimate whether the current
    recompute-starts loss is dominated by duplicated prefix math or token apply
    so the next compact-WY kernel boundary is evidence-driven.
    """

    if torch is None:
        raise RuntimeError("dplr_compact_wy_recompute_phase_probe_triton requires torch")
    normalized_phase = str(phase).strip().lower().replace("-", "_")
    if normalized_phase in {"prefix", "prefix_only", "prefix_probe"}:
        mode = 0
    elif normalized_phase in {"token", "token_apply", "token_apply_only", "apply"}:
        mode = 1
    else:
        raise ValueError("phase must be one of: prefix, token_apply")
    ...
    if not use_triton:
        if normalized_phase in {"prefix", "prefix_only", "prefix_probe"}:
            ...
            for chunk_id in range(chunks):
                cur = cur0
                for prefix_chunk in range(chunks):
                    next_cur = _compact_apply_transition_to_state(
                        cur,
                        diag[:, prefix_chunk],
                        trans_left[:, prefix_chunk],
                        trans_right[:, prefix_chunk],
                    )
                    next_cur = next_cur + _compact_outer_to_dense(
                        add_left[:, prefix_chunk].float(),
                        add_right[:, prefix_chunk].float(),
                    )
                    if prefix_chunk < chunk_id:
                        cur = next_cur
                cur_rows.append(cur.sum(dim=-1))
            return torch.stack(cur_rows, dim=1)

        start_states = state.unsqueeze(1).expand(B, chunks, H, N, N).contiguous()
        out = dplr_dense_chunk_apply_output_triton(
            r4.reshape(B, T, H * N) if flat else r4,
            w4,
            k4,
    ...
    out = torch.empty((B, T, H, N), device=r4.device, dtype=r4.dtype)
    final_state = torch.empty_like(state_c)
    prefix_probe = torch.empty((B, chunks, H, N), device=r4.device, dtype=torch.float32)
    row_blocks = triton.cdiv(N, block_m)
    _compact_wy_recompute_phase_probe_kernel[(B * chunks * H * row_blocks,)](
        r_c,
        w_c,
        k_c,
        v_c,
...
def dplr_compact_wy_grouped_prefix_shared_apply_output_triton(
    r: Any,
    w: Any,
    k: Any,
    v: Any,
    ...
    *,
    ...
    """Grouped prefix-shared compact apply/output schedule.

    This is the bounded parallelism-recovery experiment for the compact-WY
    track.  It partitions chunks into fixed-size groups.  Each group computes
    its start state once from compact prefix factors and immediately consumes
    that state for `group_size` chunks of token apply/output.  It avoids dense
    `start_states` materialization while exposing more chunk-level parallelism
    than the fully serial prefix-shared probe.
    """

    if torch is None:
        raise RuntimeError("dplr_compact_wy_grouped_prefix_shared_apply_output_triton requires torch")
    chunk_size_i = _validate_chunk_size(chunk_size)
    try:
        group_size_i = int(group_size)
    except Exception as exc:  # pragma: no cover - defensive
        raise TypeError("group_size must be an integer") from exc
    if group_size_i <= 0:
    ...
    chunks = int(diag.shape[1])
    R = int(trans_left.shape[-1])
    if chunks % group_size_i != 0:
        raise ValueError(f"chunks={chunks} must be divisible by group_size={group_size_i} for grouped prefix-shared probe")
    groups = chunks // group_size_i
    if (
        int(diag.shape[0]) != B
        or int(diag.shape[2]) != H
        or int(diag.shape[3]) != N
    ...
    if T != chunks * chunk_size_i:
        raise ValueError("tokens must equal chunks * chunk_size")
    if block_m is None:
        block_m = _env_int("RWKV7_DPLR_TRITON_GROUPED_PREFIX_SHARED_BLOCK_M", _env_int("RWKV7_DPLR_TRITON_APPLY_BLOCK_M", 8))
    if block_n is None:
        block_n = N
    block_m = int(block_m)
    block_n = int(block_n)
    ...
    final_state = torch.empty_like(state_c)
    row_blocks = triton.cdiv(N, block_m)
    _compact_wy_grouped_prefix_shared_apply_output_kernel[(B * groups * H * row_blocks,)](
        r_c,
        w_c,
        k_c,
        v_c,
...
def dplr_compact_wy_three_stage_triton(
    ...
    *,
    ...
    if prefix_shared:
        prefix_shared_group_size = _env_int("RWKV7_DPLR_TRITON_COMPACT_PREFIX_SHARED_GROUP_SIZE", 0)
        if prefix_shared_group_size > 0:
            ...
            return dplr_compact_wy_grouped_prefix_shared_apply_output_triton(
                r4.reshape(B, int(r4.shape[1]), H * N) if flat else r4,
                w4,
                k4,
                v4,
                ...
                summary,
                chunk_size=chunk_size,
                group_size=prefix_shared_group_size,
                force_fallback=force_fallback,
            )
