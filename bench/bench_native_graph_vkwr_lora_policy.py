#!/usr/bin/env python3
# coding=utf-8
"""A/B benchmark for the VKWR-inspired native_graph fused-LoRA dispatcher.

The policy is deliberately opt-in:

    RWKV7_NATIVE_GRAPH_FUSED_LORA_POLICY=vkwr_auto

It borrows VKWR's low-rank dispatch rule of using grouped W/A/G(/V) kernels
only for tiny row counts and sufficiently large hidden sizes.  This benchmark
compares that automatic policy against the historical manual/no-fused-LoRA
baseline while keeping the rest of the native_graph decode path unchanged.
"""
...
def _set_common_env(args: argparse.Namespace) -> None:
    ...
    # The policy should be the only thing deciding WAG/WAVG LoRA usage.
    ...
    os.environ["RWKV7_NATIVE_GRAPH_VKWR_LORA_MIN_HIDDEN"] = str(args.policy_min_hidden)
    os.environ["RWKV7_NATIVE_GRAPH_VKWR_LORA_MAX_ROWS"] = str(args.policy_max_rows)


def run_policy(model, token: Any, base_state: Any, args: argparse.Namespace, *, policy: str) -> dict[str, Any]:
    _set_common_env(args)
    os.environ["RWKV7_NATIVE_GRAPH_FUSED_LORA_POLICY"] = policy
...
def main() -> int:
    ...
    ap.add_argument("--policy-min-hidden", type=int, default=1024)
    ap.add_argument("--policy-max-rows", type=int, default=4)
    ap.add_argument("--baseline-policy", default="manual", choices=["manual", "off"])
    ap.add_argument("--candidate-policy", default="vkwr_auto", choices=["vkwr_auto", "vkwr", "auto"])
    ...
    with torch.inference_mode():
        _set_common_env(args)
        os.environ["RWKV7_NATIVE_GRAPH_FUSED_LORA_POLICY"] = args.baseline_policy
    ...
    row = {
        "axis": "native_graph_vkwr_lora_policy",
        "backend": "hf_adapter",
        "status": "pass",
        "dtype": args.dtype,
