...
def analyze(rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    native_graph_vkwr_lora_policy = latest(rows, lambda r: r.get("axis") == "native_graph_vkwr_lora_policy" and r.get("backend") == "hf_adapter")
    ...
    if native_graph_vkwr_lora_policy is None:
        focus.append("VKWR-style fused LoRA dispatch policy row pending")
    else:
        policy_speedup = native_graph_vkwr_lora_policy.get("speedup")
        greedy_match = native_graph_vkwr_lora_policy.get("greedy_match")
        greedy_total = native_graph_vkwr_lora_policy.get("greedy_total")
        hidden_size = native_graph_vkwr_lora_policy.get("hidden_size")
        if policy_speedup is not None and float(policy_speedup) >= 1.0:
            focus.append(
                f"VKWR-style fused LoRA dispatch hidden={hidden_size} passes greedy {greedy_match}/{greedy_total} "
                f"with speedup={float(policy_speedup):.2f}x; sweep larger checkpoints before defaulting"
            )
        elif policy_speedup is not None:
            focus.append(
                f"VKWR-style fused LoRA dispatch hidden={hidden_size} passes greedy {greedy_match}/{greedy_total} "
                f"but speedup={float(policy_speedup):.2f}x; keep policy opt-in and tune thresholds"
            )
    ...
    return {
        ...
        "native_graph_vkwr_lora_policy": compact(native_graph_vkwr_lora_policy, ["_lineno", "status", "dtype", "device", "batch_size", "hidden_size", "prompt_tokens", "steps", "fixed_token", "policy_min_hidden", "policy_max_rows", "fused_recurrent_enabled", "fused_recurrent_output_enabled", "fused_output_enabled", "fused_output_project_enabled", "block_m", "block_r", "block_k", "baseline_policy", "candidate_policy", "baseline_effective_backend", "candidate_effective_backend", "baseline_ms_per_step", "candidate_ms_per_step", "speedup", "baseline_tokps_total", "candidate_tokps_total", "max_abs_diff_first_step", "min_cosine_first_step", "greedy_match", "greedy_total", "baseline_cache_stats", "candidate_cache_stats", "peak_vram_mb"]),
...
def print_text(report: dict[str, Any]) -> None:
    ...
    print("\n## native_graph_vkwr_lora_policy")
    print(json.dumps(report["native_graph_vkwr_lora_policy"], ensure_ascii=False) if report["native_graph_vkwr_lora_policy"] else "PENDING")
    print("\n## native_graph_fused_projection")
