...
def test_dense_chunk_summary_torch_final_state_matches_recurrent_scan() -> None:
    ...
    recompute_out, recompute_state = dplr_compact_wy_recompute_apply_output_triton(
        r, w, k, v, kk, a, state, compact, chunk_size=4, force_fallback=True
    )
    assert torch.allclose(recompute_out, ref_out, atol=2e-6, rtol=2e-6), (recompute_out - ref_out).abs().max()
    ...
    prefix_shared_out, prefix_shared_state = dplr_compact_wy_prefix_shared_apply_output_triton(
        r, w, k, v, kk, a, state, chunk_size=4, force_fallback=True
    )
    assert torch.allclose(prefix_shared_out, ref_out, atol=2e-6, rtol=2e-6), (
        prefix_shared_out - ref_out
    ).abs().max()
    assert torch.allclose(prefix_shared_state, ref_state, atol=2e-6, rtol=2e-6), (
        prefix_shared_state - ref_state
    ).abs().max()
    compact3_prefix_shared_out, compact3_prefix_shared_state = dplr_compact_wy_three_stage_triton(
        r, w, k, v, kk, a, state, chunk_size=4, prefix_shared=True, force_fallback=True
    )
    assert torch.allclose(compact3_prefix_shared_out, ref_out, atol=2e-6, rtol=2e-6), (
        compact3_prefix_shared_out - ref_out
    ).abs().max()
    ...
    assert torch.allclose(compact3_recompute_out, ref_out, atol=2e-6, rtol=2e-6), (
        compact3_recompute_out - ref_out
    ).abs().max()
    assert torch.allclose(compact3_recompute_state, ref_state, atol=2e-6, rtol=2e-6), (
        compact3_recompute_state - ref_state
    ).abs().max()
...
def test_dense_chunk_summary_triton_matches_torch_cuda() -> None:
    ...
    recompute_out, recompute_state = dplr_compact_wy_recompute_apply_output_triton(
        r, w, k, v, kk, a, state, compact_got, chunk_size=4
    )
    assert torch.allclose(recompute_out, ref_out, atol=2e-6, rtol=2e-6), (recompute_out - ref_out).abs().max()
    assert torch.allclose(recompute_state, ref_state, atol=2e-6, rtol=2e-6), (
        recompute_state - ref_state
    ...
    assert token_probe.shape == ref_out.shape
    prefix_shared_out, prefix_shared_state = dplr_compact_wy_prefix_shared_apply_output_triton(
        r, w, k, v, kk, a, state, chunk_size=4, block_m=2
    )
    assert torch.allclose(prefix_shared_out, ref_out, atol=2e-6, rtol=2e-6), (
        prefix_shared_out - ref_out
    ).abs().max()
    ...
    compact3_prefix_shared_out, compact3_prefix_shared_state = dplr_compact_wy_three_stage_triton(
        r, w, k, v, kk, a, state, chunk_size=4, prefix_shared=True
    )
    assert torch.allclose(compact3_prefix_shared_out, ref_out, atol=2e-6, rtol=2e-6), (
        compact3_prefix_shared_out - ref_out
    ).abs().max()
